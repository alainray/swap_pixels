...
EXP_NAME="mpi3d_${model}_${method}_${STATUS}_full"

echo "Iniciando Job para Experimento: $EXP_NAME | Seed: $seed"
python train.py --dataset mpi3d --idg_root /workspace1/asoto/araymond/datasets \
  --idg_split composition \
  --out_dir runs/ \
  --run_name "$EXP_NAME" \
  --inv_factors robot_arm_horizontal \
  --device cuda \
  --encoder $model \
  --z_dim 256 \
  --train_mode $method \
